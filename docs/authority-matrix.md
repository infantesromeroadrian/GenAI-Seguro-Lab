...
15. `CMP-17` puede verificar `DAT-20/21/22/23`, pero no genera hallazgos,
    ...
    final el retest.
16. `CMP-18` puede materializar el candidato final fijado bajo `$TMP`,
    ejecutar los 14 casos adversarios y 12 benignos, y evaluar después sus
...
```text
ACT-02 (soporte operativo M04)
  → CMP-16 verifica commits, árboles y cuatro entradas comunes
  → copias git archive bajo $TMP
  → 3 pares warmup + 30 pares AB/BA de main.py baseline
...
tratamiento. La decimotercera repite el candidato final exacto con rúbrica
separada y conserva únicamente una proyección revisable, sin añadir una ruta
de producto.
