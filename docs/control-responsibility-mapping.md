...
| `CTL-04` | Separación de instrucciones y contenido no confiable, resistencia a inyección y jailbreak | `PLANIFICADO`; prueba PI presente | `A/R ACT-02` | `AC-PI-01`, `AC-PI-02`, `AC-PI-03`, `AC-JB-01` | `CMP-07` demuestra el rechazo de prompt libre y el comportamiento determinista ante dos inyecciones indirectas; no existe todavía el control endurecido ni evidencia sobre un modelo GenAI real | PGS-04-M01 y retest PGS-05-M01 a M03 |
...
| `CTL-12` | Harness adversario, métricas, regresión y revisión independiente | `PARCIAL` | `A/R ACT-02`; `R REV-01` solo para revisión independiente | Los 17 casos de `GSL-ABUSE-CASES-001` | `CMP-07` y pytest cubren los tres casos PI con oráculos separados; faltan 14 abuse cases, baseline de seguridad, métricas agregadas, retest y revisor independiente | PGS-03-M05 a M08, PGS-05 y PGS-07-M01 a M06 |
...
y `CMP-07` cubre los tres casos PI sin convertir ese resultado en una baseline
canónica. PGS-03-M05 debe implementar jailbreak y revelación de información.
