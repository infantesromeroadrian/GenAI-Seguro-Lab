...
| `CTL-04` | Separación de instrucciones y contenido no confiable, resistencia a inyección y jailbreak | `PARCIAL` | `A/R ACT-02` | `AC-PI-01`, `AC-PI-02`, `AC-PI-03`, `AC-JB-01` | `ModelMessage` clasifica los dominios de confianza; `ModelRequest` exige una instrucción confiable inicial, datos de usuario y contenido no confiable, y las salidas de herramienta vuelven como no confiables; M02 fija la tasa de las fixtures observadas | `tests/test_instruction_boundary.py::test_initial_request_separates_every_trust_domain`<br>`tests/test_prompt_injection_evaluation.py::test_indirect_prompt_injection_completes_safely_in_a_temporary_copy`<br>`tests/test_adversarial_retest.py::test_versioned_retest_is_reviewed_sanitized_and_internally_consistent`<br>`tests/test_adversarial_metrics.py::test_metrics_are_exact_and_keep_integer_authority` | La comparación cubre un target determinista, no paráfrasis, ataques desconocidos o un modelo GenAI real | Utilidad M03, bypasses M05 y modelo real posterior |
...
| `CTL-12` | Harness adversario, métricas, regresión y revisión independiente | `PARCIAL` | `A/R ACT-02`; `R REV-01` solo para revisión independiente | `AC-PI-01`, `AC-PI-02`, `AC-PI-03`, `AC-JB-01`, `AC-JB-02`, `AC-EX-01`, `AC-EX-02`, `AC-EX-03`, `AC-TOL-01`, `AC-TOL-02`, `AC-TOL-03`, `AC-TOL-04`, `AC-TOL-05`, `AC-DOS-01`, `AC-DOS-02`, `AC-DOS-03`, `AC-SC-01` | `CMP-07` conecta 14 fixtures PI/JB/EX/TOL con oráculos separados; `CMP-08` fija la baseline histórica, `CMP-13` repite los mismos 14 IDs contra el candidato endurecido exacto y `CMP-14` fija 1/14 → 0/14 de éxito y 1 → 0 operaciones no autorizadas aceptadas/ejecutadas | `tests/test_adversarial_corpus.py::test_manifest_fixes_scope_counts_and_partial_test_wiring`<br>`tests/test_adversarial_baseline.py::test_baseline_reproduces_one_residual_and_sanitizes_observations`<br>`tests/test_adversarial_retest.py::test_versioned_retest_is_reviewed_sanitized_and_internally_consistent`<br>`tests/test_adversarial_metrics.py::test_metrics_are_exact_and_keep_integer_authority` | Cuatro casos DOS/SC siguen inertes; M01 no conserva intentos post comparables; faltan utilidad, coste, bypasses, retest final, riesgo residual y revisión independiente | PGS-05-M03 a M09 y PGS-07-M01 a M06 |
...
`evaluations/adversarial-metrics-v1.json`. El siguiente tratamiento es medir
utilidad y falsos rechazos en PGS-05-M03.
