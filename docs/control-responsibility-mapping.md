...
| `CTL-11` | Integridad de código, dependencias, cambios y releases | `PARCIAL` | `A/R ACT-02`; `C REV-01` planificado | `AC-DOS-02`, `AC-SC-01` | Git, remoto público, `uv.lock`, hashes del corpus y commits granulares permiten detectar diferencias en los artefactos cubiertos; [`GSL-SUPPLY-CHAIN-001`](./dependency-supply-chain-register.md) registra 11 distribuciones, toolchain y ocho gaps | `tests/test_adversarial_baseline.py::test_run_rejects_candidate_drift`<br>`tests/test_adversarial_baseline.py::test_versioned_evidence_is_reviewed_sanitized_and_internally_consistent`<br>`tests/test_dependency_supply_chain_register.py::test_lock_and_register_cover_all_external_packages_with_hashes` | No existen firma, CI, SBOM, revisión independiente ni política de release; licencias y advisories no están evaluados y las pruebas no cubren toda la cadena de suministro | PGS-07-M01/M03/M04 |
